...
public class BlockQuern extends Block implements IItemSize, IHighlightHandler {
    ...
    private static final AxisAlignedBB INPUT_SLOT_AABB = new AxisAlignedBB(0.375D, 0.86D, 0.375D, 0.625D, 1.0D, 0.625D);
                ...
                if (inventory != null) {
                    if (!teQuern.isGrinding()) {
                        if (selection == SelectionPlace.HANDLE) {
                            ...
                            playerIn.setHeldItem(EnumHand.MAIN_HAND, teQuern.insertOrSwapItem(SLOT_INPUT, heldStack));
